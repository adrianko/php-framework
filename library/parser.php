...
class Parser {
    ...
    public function clean($string) {
        $string = nl2br(htmlentities(trim($string)));
    }
    ...
    public function youtubeUrl($url) {
    ...
    public function removeUnsafeAttributes() {
        ...
        $this->string = preg_replace($unsafe, "", $this->string);
    }

    public function removeNumbers() {
        $this->string = str_replace(range(0, 9), '', $this->string);
    }
    ...
    public function selective() {
        ...
        $this->string = strip_tags($this->string, '<strong><em><a><p><br><img><ul><ol><li><span><iframe><pre><blockquote><h2><h3>');
        $this->string = $this->removeUnsafeAttributes($this->string);
        $shd          = new simple_html_dom();
        $shd->load($this->string);
        $img = $shd->find('img');
        for ($i = 0; $i < count($img); $i++) {
            if ($this->imageUrl($img[$i]->src) == true || $this->imageUrl("http://" . $_SERVER['HTTP_HOST'] . $img[$i]->src) == true) {
                $oldimg = $img[$i]->outertext;
                list($width, $height) = @getimagesize("http://" . $_SERVER['HTTP_HOST'] . $img[$i]->src);
                list($width, $height) = @getimagesize($img[$i]->src);
                if (property_exists($img[$i], "width") == false) {
                    $img[$i]->width = $width;
                }
                if (property_exists($img[$i], "height") == false) {
                    $img[$i]->height = $height;
                }
                if ($this->removeLetters($img[$i]->width) > $this->max_width) {
                    $img[$i]->width  = $this->max_width;
                    $height          = ($this->max_width * $height) / $width;
                    $img[$i]->height = $height;
                }
                if (property_exists($img[$i], "title") == true) {
                    $img[$i]->title = $this->clean($img[$i]->title);
                }
                if (property_exists($img[$i], "longdesc") == true) {
                    if ($this->url($img[$i]->longdesc) == false) {
                        $img[$i]->longdesc = "";
                    }
                }
                $this->string = str_replace($oldimg, $img[$i]->outertext, $this->string);
            } else {
                $this->string = str_replace($img[$i]->outertext, "", $this->string);
            }
        }
        $a = $shd->find('a');
        for ($j = 0; $j < count($a); $j++) {
            if ($this->url($a[$j]->href) == true) {
                $olda = $a[$j]->outertext;
                if (property_exists($a[$j], "target") == false || $a[$j]->target != "_blank") {
                    $a[$j]->target = "_blank";
                }
                if (property_exists($a[$j], "rel") == false || $a[$j]->rel != "nofollow") {
                    $a[$j]->rel = "nofollow";
                }
                $this->string = str_replace($olda, $a[$j]->outertext, $this->string);
            } else {
                $this->string = str_replace($a[$j]->outertext, "", $this->string);
            }
        }
        $iframe = $shd->find('iframe');
        for ($k = 0; $k < count($iframe); $k++) {
            if ($this->youtubeUrl($iframe[$k]->src) == true) {
                $oldiframe = $iframe[$k]->outertext;
                if (property_exists($iframe[$k], "width") == false) {
                    $iframe[$k]->width = 560;
                }
                if (property_exists($iframe[$k], "height") == false) {
                    $iframe[$k]->height = 315;
                }
                if ($this->removeLetters($iframe[$k]->width) > 560) {
                    $iframe[$k]->width  = 560;
                    $iframe[$k]->height = 315;
                }
                $this->string = str_replace($oldiframe, $iframe[$k]->outertext, $this->string);
            } else {
                $this->string = str_replace($iframe[$k]->outertext, "", $this->string);
            }
        }
